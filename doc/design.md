...
- **始業ボタンの配置と1日1回制限**: 誤操作を防ぐため「始業」ボタンをアクションバーの左端に配置し、右端の「保存して閉じる」ボタンと距離を取っています。また、1日に複数回始業が記録されるのを防ぐため、起動時に本日分のログ履歴を検査し、すでに `[始業]` タグのエントリがある場合は「始業」ボタンを無効化（disabled）します。
